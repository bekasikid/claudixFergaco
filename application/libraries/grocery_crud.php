...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->edit_url				= $this->getEditUrl();
                $data->view_url                         = $this->getViewUrl(); /* tambahan qq */
		...
		$data->unset_view			= $this->unset_view; /* tambahan qq */
...
class grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $unset_view			= false; /*tambahan qq*/
	...
	public function set_width($field_name, $width = null)
	{
		if(is_array($field_name))
		{
			foreach($field_name as $field => $width)
			{
				...
				elseif(isset($field_types[$column]))
					...
					$this->columns[$col_num] = (object)array('field_name' => $column, 'display_as' => 
						ucfirst(str_replace('_',' ',$column)));
                                ...
                                if(isset($this->width[$column]))
                                    $this->columns[$col_num]->width = $this->width[$column];
                                else 
                                    $this->columns[$col_num]->width = '';
