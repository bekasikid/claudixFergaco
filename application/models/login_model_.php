...
//                 $this->db->where('IS_LOGIN',0);	
		...
		if ($q->num_rows == 1) {	// if query had 1 row to return
                    ...
                    if($data_user->user_status == '1' && $data_user->is_login == '0') {
                        ...
                        $access = $this->mitra_model->get_menu_role($data_user->id_modul_role);
                    	...
                    	$data = array('login'=>'hore','user_info'=>$data_user,'menu'=>$data_menu,'opr'=>'test','access'=>$dt_access);
                    	return $data;
                    } else{
